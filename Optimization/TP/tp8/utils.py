...
def plot_obj_supportSize(x_tab, F, lambda1, title):


    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True)
    fig.suptitle(title)

    ax1.semilogy([F(x_tab[k,:], lambda1) for k in range(x_tab.shape[0])])
    ax1.set_ylabel(r'$F(x_k)$')

    ax2.semilogy([sum(x_tab[k,:] != 0) for k in range(x_tab.shape[0])])
    ax2.set_xlabel(r'$k$')
    ax2.set_ylabel(r'$\|x_k \|_0$')
